...
public class WordsForNumbers {
	...
	public static void main(String[] args) {
		...
		String t = " ";
		List<String> listOfSent = new ArrayList<>();
		while (t.length() > 0) {
			t = scan.nextLine();
			if (t.length() > 0)
				listOfSent.add(t);
		...
		listOfSent.forEach(sen -> {
			String[] split = sen.split(" ");

			List<String> list = new LinkedList<>();
			list.addAll(Arrays.asList(split));
			String firstWord = list.get(0);
			AtomicReference<String> finalSent = new AtomicReference<>("");
			if (isStringNumeric(firstWord)) {
				...
				if (finalVal.contains(" ")) {
					...
					finalSent.set(finalSent.get().concat(finalVal + " "));
				} else {
					finalSent.set(finalSent.get().concat(finalVal + " "));
				}

			} else {
				finalSent.set(finalSent.get().concat(firstWord + " "));
			}
			list.remove(0);
			...
			list.forEach(word -> {
				if (isStringNumeric(word)) {
					...
					finalSent.set(finalSent.get().concat(word + " "));
				}

			});
			finalList.add(finalSent.get());
		});
